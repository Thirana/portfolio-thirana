...
After that, the remaining question is: what `cache-aside` workflow keeps speed high without breaking correctness?

## The Caching Pattern

A common strategy is `cache-aside`:

- **Read cache first** — check the cache before hitting the database.
- **Fall back to DB on miss** — query the source of truth only when the cache has no answer.
- **Warm the cache** — store the DB result in cache for future requests.
...
sequenceDiagram
  ...
  else cache miss, missing or expired
    rect rgb(61, 49, 20)
      API->>DB: query by code
...
> These results come from the same load scenario (`VUS=20`, `DURATION=20s`) run before and after enabling Redis `cache-aside`. Error rate stayed at 0% throughout.
...
- **Speed vs invalidation complexity** — Cache improves speed but increases key, `TTL`, and invalidation complexity.
- **Negative caching vs TTL discipline** — Negative caching reduces wasted DB reads, but `TTL` must be chosen carefully to avoid stale "not found" entries.
- **Fallback is required** — Fallback logic is required before cache can be trusted in production.
- **Validate with load tests** — Performance claims should be validated with repeatable load tests, not assumed from theory.
...
If you want to see how create-request retries are handled safely, read [Idempotency in API Design: How to Make Retries Safe](/blog/idempotency-in-api-design).
